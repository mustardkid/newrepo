    ...
    return (
      <div className="min-h-screen bg-deep-gradient flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white admin-text">Loading AI highlights...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-deep-gradient flex items-center justify-center">
        <Card className="glass-card bg-card-gradient border-white/10 p-8 max-w-md">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-white mb-2 admin-heading">Error Loading Highlights</h2>
            <p className="text-white/70 mb-4 admin-text">Unable to load AI processed videos. Please try again.</p>
            <Button 
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-white admin-heading">AI Highlights Review</h1>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              {selectedVideos.size > 0 && (
                <div className="flex items-center space-x-2">
                  <span className="text-white/70 text-sm admin-text">
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <CardTitle className="text-lg font-semibold text-white line-clamp-2 admin-heading">
                        {video.contentAnalysis?.cleanedText || video.title}
                      </CardTitle>
                      <p className="text-white/60 text-sm mt-1 admin-text">
              ...
              <CardContent className="pt-0">
                ...
                <div className="aspect-video bg-black/20 rounded-lg mb-4 relative overflow-hidden">
                  {video.thumbnailUrl ? (
                    <img 
                      ...
                      alt={video.title}
                      className="admin-thumbnail-large w-full h-full object-cover"
                    />
                  ) : (
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    {video.contentAnalysis?.containsProfanity && (
                      <Badge className={`${getSeverityBadgeColor(video.contentAnalysis.severityLevel)} text-white text-xs admin-text`}>
                        {video.contentAnalysis.severityLevel.toUpperCase()}
                      </Badge>
                    )}
                    {video.contentAnalysis?.containsHateSpeech && (
                      <Badge className="bg-red-600 text-white text-xs admin-text">
                        HATE SPEECH
                      </Badge>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="text-center">
                    <div className="text-white font-semibold admin-text">
                      {video.duration ? `${Math.floor(video.duration / 60)}:${(video.duration % 60).toString().padStart(2, '0')}` : 'N/A'}
                    </div>
                    <div className="text-white/50 text-xs admin-text">Duration</div>
                  </div>
                  <div className="text-center">
                    <div className="text-white font-semibold admin-text">
                      {video.processedHighlights?.length || 0}
                    </div>
                    <div className="text-white/50 text-xs admin-text">Highlights</div>
                  </div>
                  <div className="text-center">
                    <div className="text-white font-semibold admin-text">
                      {video.status}
                    </div>
                    <div className="text-white/50 text-xs admin-text">Status</div>
                  </div>
                </div>
                ...
                <div className="flex space-x-2">
                  <Button 
                    size="sm" 
                    className="flex-1 admin-btn-approve"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <Button 
                    size="sm" 
                    className="flex-1 admin-btn-reject"
                    onClick={(e) => {
                      e.stopPropagation();
        ...
        {filteredVideos.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2 admin-heading">No Videos Found</h3>
            <p className="text-white/60 admin-text">
              {filterLevel === 'flagged' 
